...
class GeneratorResnet(nn.Module):
    ...
    def forward(self, input):
        ...
        x = (torch.tanh(x) + 1) / 2
        if self.evaluate:
            x_0 = torch.where(x < 0.95, torch.zeros_like(x).detach(), torch.ones_like(x).detach())
        else:
            x_0 = torch.where(torch.rand(x.shape).cuda() < 0.95, x,
                              torch.where(x < 0.95, torch.zeros_like(x), torch.ones_like(x)).detach())
        x_out = x_inf * x_0 + input
